...
export const Messages = {
  ...
  ScheduledRecurringJob: (jobName: string) =>
    `Scheduled recurring job ${jobName}`,
  ErrorSchedulingRecurringJob: (jobName: string, error: any) =>
    `Error scheduling recurring job ${jobName}: ${error}`,
  FailedJobNoJobInstance: (message: string) =>
    `Failed job with no job instance. \nError: ${message}`,
  ManuallyTriggeredJob: (jobName: string) =>
    `Manually triggered job ${jobName}`,
  FailedToTriggerJob: (jobName: string, error: any) =>
    `Failed to trigger job ${jobName}: ${error}`,
